...
- **Agent 模式**：基于 Pi SDK（`@earendil-works/pi-coding-agent`）的通用 Agent，支持工作区隔离、权限模式、文件操作、长任务流式输出、计划确认和用户追问。
- **SubAgent / Tasks**：复杂任务可以通过 Pi runtime 的任务工具拆分为子 Agent / Task，并在消息流中展示调用过程和结果。
...
从 [GitHub Releases](https://github.com/ErlichLiu/Proma/releases) 下载开源版本。发布页提供 macOS Apple Silicon、macOS Intel 和 Windows 安装包，请以 GitHub Releases 中的最新版本说明为准。
...
4. Agent 模式当前需要 Anthropic 协议或 Anthropic 兼容协议渠道，例如 Anthropic、DeepSeek、Kimi API、Kimi Coding Plan、MiniMax、通义千问 Anthropic 协议、智谱 Coding Plan 或小米 MiMo。普通 OpenAI / Google / 豆包 / 通义千问 / 自定义 OpenAI 兼容端点当前仅用于 Chat。
...
| Anthropic 兼容端点 | 支持 | 支持 | 用户填写完整 Messages 请求地址 |
...
| 智谱 AI | 支持 | 暂不支持 | OpenAI 兼容协议 |
| 智谱 Coding Plan | 支持 | 支持 | Anthropic 兼容协议 |
| MiniMax | 支持 | 支持 | Anthropic 兼容协议 |
| 豆包 | 支持 | 暂不支持 | OpenAI 兼容协议 |
| 通义千问 | 支持 | 暂不支持 | OpenAI 兼容协议 |
| 通义千问 (Anthropic 协议) | 支持 | 支持 | DashScope Anthropic 兼容协议 |
| 小米 MiMo API | 支持 | 支持 | Anthropic 兼容协议 |
| 小米 MiMo Token Plan | 支持 | 支持 | Anthropic 兼容协议，使用专用认证头 |
| 自定义端点 | 支持 | 暂不支持 | OpenAI 兼容协议，用户填写完整 Chat Completions 请求地址 |

> **Kimi Coding Plan 用户须知**：Proma 已获得 Kimi 官方白名单支持，使用 Proma 连接 Kimi Coding Plan 不会触发第三方客户端封号策略，可放心使用。

Agent 模式底层使用 Pi SDK runtime，并通过 Proma 渠道配置传入模型、Base URL 和 API Key。Chat 模式则通过 `@proma/core` 的 Provider Adapter 统一接入不同协议。
...
│   ├── session-core/ # Agent 会话读取、分组、搜索、转录与渲染核心
...
主要 workspace 包（版本以各自 `package.json` 为准）：

| 包 | 职责 |
| --- | --- |
| `@proma/electron` | Electron 桌面应用 |
| `@proma/shared` | 共享类型、IPC 常量、配置和工具 |
| `@proma/session-core` | Agent 会话读取、分组、搜索、转录与渲染核心 |
| `@proma/core` | Provider Adapter、SSE、Shiki 高亮 |
| `@proma/ui` | 共享 React UI 组件 |
| `@proma/cli` | 面向有限上下文消费者的会话读取 CLI |
...
Pi SDK runtime 由 `@earendil-works/pi-coding-agent`、`@earendil-works/pi-agent-core` 和 `@earendil-works/pi-ai` 提供。Proma 的主进程 esbuild 会把这些包标记为 external，打包前通过 `bun run sync:runtime-deps` 把运行时依赖闭包同步到 `apps/electron/node_modules/`。

修改打包配置时请特别确认：

- 主进程 esbuild 保持 `@earendil-works/pi-*` runtime 包 external。
- `apps/electron/scripts/sync-runtime-deps.ts` 会先重建 `apps/electron/node_modules/`，再按 allowlist 同步 Pi runtime 依赖闭包，并检查旧 `@anthropic-ai/claude-agent-sdk*` 残留。
- `apps/electron/electron-builder.yml` 打包同步后的 `node_modules/**/*`，并解包 Pi / PDF.js 可能用到的 native addon。
