...
- **入口文件**：\`.claude/memory/MEMORY.md\` 只放主题索引和路由；详细内容拆到同目录或子目录下的主题文件，都会被一并注入
...
6. **自检习惯**：复杂任务执行过程中，定期回顾相关的 CLAUDE.md、auto memory、Skills 和两级 .context/ 内容，确保行为与已记录的规范、经验和计划保持一致
