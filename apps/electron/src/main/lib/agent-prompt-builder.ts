 ...
 * 工具（Read/Write/Edit/Bash 等）由 Pi runtime 独立注册，不受 systemPrompt 影响。
 */
...
代码审查请使用 Proma 内置的 \`/code-review\` 或 \`/simplify\` Skill`)
...
工作区级 auto memory 目录位于工作区根目录的 \`.claude/memory/\`。Proma 会在每条消息前把该目录的内容实时读取并注入到 \`<workspace_memory>\` 上下文块中，因此写入这里的内容会在后续所有会话中自动出现在你眼前：
- **用途**：沉淀跨会话学习到的项目经验、用户偏好和易错点
- **入口文件**：\`.claude/memory/MEMORY.md\` 是索引；更多主题文件放在同目录或子目录，都会被一并注入
- **写入方式**：用 Write/Edit 直接写入该目录下的文件；无需任何额外命令，写完下一条消息即可被自动加载
- **使用要求**：不要把它当聊天流水账；写入前确认信息对未来会话有稳定价值。内容体量控制在注入预算内，超出部分需靠 Read 按需读取
...
| 需要让 Agent 未来自动回忆的稳定经验 | → 更新 .claude/memory/MEMORY.md 或主题文件 |
...
6. **自检习惯**：复杂任务执行过程中，定期回顾 CLAUDE.md、auto memory 和两级 .context/ 中的内容，确保行为与已记录的规范和计划保持一致
...
export function buildDynamicContext(ctx: DynamicContext): string {
  ...
  if (ctx.workspaceSlug) {
    ...
    // auto memory（.claude/memory/）实时注入
    ...
    if (memoryBlock) {
      sections.push(`<workspace_memory>\n以下是本工作区的 auto memory（\`.claude/memory/\`，跨会话自动记忆，每条消息实时读取）。把它作为背景知识，遇到与之相关的任务时主动参考；发现有稳定价值的新经验时用 Write 更新对应文件。\n\n${memoryBlock}\n</workspace_memory>`)
    }
  }
